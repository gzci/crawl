...
public class Tielu4Parse implements ChainHandler {

	EntityDao dao = new EntityDao(Tabtielu4.class);
	...
	public Boolean apply(JSONObject task, Site site) {
		...
		if (StringUtils.isNotBlank(url) && url.matches("http://.+/show.*")) {
			site.wd().get(url);
			...
			// 获得公司信息并入库
			Tabtielu4 t = new Tabtielu4();
			t.url = url;
			t.anchor_text = pageSource;
			t.context = JSOUPUtils.find(dom, "div.pt30").map(e -> e.text()).orElse(null);
			t.remark ="铁路博览-高铁";
			t.titile=  JSOUPUtils.find(dom, "#article h2").map(e -> e.text()).orElse(null);
