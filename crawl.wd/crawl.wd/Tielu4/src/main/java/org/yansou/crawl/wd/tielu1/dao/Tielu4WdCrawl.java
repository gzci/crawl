...
//		PhantomJSDriver wd = new PhantomJSDriver(capabilities);
	...
	public void run(RemoteWebDriver wd) {
		...
		site.offerTask(JSON.parseObject(
				"{'_STATUS_':{'SEED':true,'LIST':true},'url':'http://www.peoplerail.com/rail/list-475-1.html'}"));
		List<ChainHandler> chainHandlerList = Arrays.asList(new Tielu4List(), new Tielu4Parse(),new ZhaopinPosition());
		for (;;) {
