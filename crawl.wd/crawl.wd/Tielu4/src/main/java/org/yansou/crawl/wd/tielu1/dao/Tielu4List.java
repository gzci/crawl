...
public class Tielu4List implements ChainHandler {
    @Override
    public Boolean apply(JSONObject task, Site site) {
        ...
        if (StringUtils.isNotBlank(url) && url.matches("http://.+/list.*")) {
            ...
            JSOUPUtils.finds(dom, "div#list_div a[href*=show]").forEach(e -> {
            ...
            newTask.put("url", "http://www.peoplerail.com/rail/"+nextUrl);
            site.offerTask(newTask);
        }
        return true;
    }
